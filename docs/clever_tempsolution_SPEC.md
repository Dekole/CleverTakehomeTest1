...
- **Upload limits:** `MAX_CONTENT_LENGTH` (~5MB) in Flask config. Uploads exceeding this are rejected immediately with a 413 error shown in the UI.
- **Upload column validation:** `_validate_csv()` in `app.py` reads only the CSV header row (no body parsing) and confirms that every required column from §3.1/§3.2 is present before the file is accepted. Missing columns produce a "File rejected:" error banner above the Inputs form; all sections below the form (Summary, Leads, Outreach, Exports) remain blank. The check uses a subset match — extra columns in the upload are allowed and ignored. Default (bundled) files bypass this check since they are known-good. Required columns: leads — `lead_id, source, received_at, first_name, last_name, phone, email, consent_sms, consent_call, property_zip, lead_type, status`; outreach — `attempt_id, lead_id, channel, attempted_at, status, agent_id`.
